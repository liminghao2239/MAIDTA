...
from core.models.graph_transformer_pytorch import GraphTransformer
...
class AttentionMatrix(nn.Module):
    ...
    def forward(self, xt, xg):
        """
        :param xt: (batch_size, 128)
        :param xg: (batch_size, 128)
        :return: attention matrix of shape (batch_size, 128, 128)
        """
        # 计算查询、键、值
        Q = self.linear_q(xt)  # (batch_size, 128)
        K = self.linear_k(xg)  # (batch_size, 128)
        V = self.linear_v(xg)  # (batch_size, 128)

        # 计算注意力分数: 点积
        attention_scores = torch.matmul(Q, K.transpose(1, 0))  # (batch_size, batch_size)

        # 使用 softmax 对注意力分数进行归一化
        attention_weights = F.softmax(attention_scores, dim=-1)  # (batch_size, batch_size)

        # 计算加权输出
        attention_output = torch.matmul(attention_weights, V)  # (batch_size, 128)

        # 生成注意力矩阵 (batch_size, 128, 128)
        attention_matrix = attention_weights.view(xt.size(0), 128, 128)

        return attention_matrix, attention_output
...
class Attention1(nn.Module):
    def __init__(self, seq_len=128, d_k=128):
        """
        初始化注意力模块
        :param seq_len: 序列长度（对应输入的第二个维度128）
        :param d_k: 注意力头的维度（这里设为128，与序列长度一致）
        """
        super(Attention1, self).__init__()
        self.seq_len = seq_len
        self.d_k = d_k  # 注意力缩放因子的分母

        # 线性层：将输入特征映射为Query、Key、Value
        # 输入的每个位置是1维特征（需先扩展维度），映射到d_k维
        self.W_q = nn.Linear(1, d_k)  # Query映射（输入维度1，输出维度d_k）
        self.W_k = nn.Linear(1, d_k)  # Key映射
        self.W_v = nn.Linear(1, d_k)  # Value映射（虽然这里只需要注意力矩阵，但保留标准结构）

    def forward(self, xt, xg):
        """
        前向传播计算注意力矩阵
        :param xt: 输入张量，形状为 (batch_size, seq_len) → (128, 128)
        :param xg: 输入张量，形状为 (batch_size, seq_len) → (128, 128)
        :return: 注意力矩阵，形状为 (batch_size, seq_len, seq_len) → (128, 128, 128)
        """
        # 1. 扩展维度：将输入从 (batch, seq_len) 转为 (batch, seq_len, 1)
        # （因为每个位置的特征是1维，需显式扩展才能通过线性层映射）
        xt_feat = xt.unsqueeze(-1)  # 形状：(128, 128, 1)
        xg_feat = xg.unsqueeze(-1)  # 形状：(128, 128, 1)

        # 2. 映射为Query、Key、Value（形状均为 (batch, seq_len, d_k) → (128, 128, 128)）
        Q = self.W_q(xt_feat)  # Query来自xt：(128, 128, 128)
        K = self.W_k(xg_feat)  # Key来自xg：(128, 128, 128)
        # V = self.W_v(xg_feat)  # 若需注意力加权输出可保留，这里仅需矩阵则注释

        # 3. 计算注意力分数：Q与K的转置点积（缩放后）
        # K转置：(128, 128, 128) → (128, 128, 128)（交换最后两维）
        scores = torch.matmul(Q, K.transpose(-2, -1))  # 形状：(128, 128, 128)
        scores = scores / torch.sqrt(torch.tensor(self.d_k, dtype=torch.float32))  # 缩放

        # 4. softmax归一化，得到注意力矩阵（每行和为1）
        attn_matrix = F.softmax(scores, dim=-1)  # 形状：(128, 128, 128)
...
class GCNEdgeNet(torch.nn.Module):
    def __init__(self, n_output=1, num_features_xd=67, xg=1280, latent_dim=128, output_dim=128, dropout=0.2, edge_input_dim=None):
        ...
        # Initialize a list to store attention matrices W for each epoch
        self.saved_attention_matrices = []
        self.recorded_W = {}  # 字典用于记录每个 epoch 的每个批次的 W 矩阵


        self.drug_branch = GraphTransformer(
            dim=128,
            depth=3,
            heads=8,
            with_feedforwards=True,
            accept_adjacency_matrix=True  # ★ 关键
        )
        ...
        self.atten1 = Attention1()
        self.cnn_linear = nn.Linear(output_dim, output_dim)

        self.attention_model = AttentionMatrix(input_dim=128)
    ...
    def forward(self, data_mol, data_prot):
        ...
        nodes, mask = to_dense_batch(x, batch)  # [B, N, F] 此处 F=67
        adj = to_dense_adj(edge_index, batch).squeeze(-1)  # [B, N, N]

        # #
        # if not hasattr(self, 'node_proj'):  # 只在首次 forward 建一次
        #     in_dim = nodes.size(-1)  # = 67  (以后再加列也能自适应)
        #     self.node_proj = nn.Linear(in_dim, 128, bias=False).to(nodes.device)
        # nodes = self.node_proj(nodes)  # 现在是 [B, N, 128]
        #
        # # ------------- Transformer -------------
        # nodes, _ = self.drug_branch(
        #     nodes,
        #     adj_mat=adj.long(),  # accept_adjacency_matrix=True 已开启
        #     mask=mask
        # )
        #
        # x = (nodes * mask.unsqueeze(-1)).sum(1) / mask.sum(1, keepdim=True)  # [B,128]
        ...
        pocket = data_prot.pocket    #torch.Size([128, 128])
        pocket = pocket.view(128, 128)
        ...
        W = self.atten3(xp)
        Wa = self.atten1(xt,xg)
        # print("wa",  Wa.shape)
        Wout= xt *W+ xg*(1-W)#128,128

        for i in range(len(target)):  # 逐个蛋白质进行处理
            attention_matrix = Wa[i].detach().cpu().numpy()  # 获取当前样本的注意力矩阵
            # print("wa",i,"show", attention_matrix.shape)
            self.saved_attention_matrices.append(attention_matrix)  # 按顺序保存每个注意力矩阵
