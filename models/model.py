...
class ESM1vEncoder(nn.Module):
    def __init__(self, embed_dim=128, context_window=1024, esm_dim=1280):
        super().__init__()
        self.context_window = context_window
        self.proj = nn.Linear(esm_dim, embed_dim)
        self.pos_enc = PositionalEncoding(embed_dim, max_len=context_window)

    def forward(self, x):
        # x: [batch, NP, esm_dim] or [batch, NP] (tokens)
        if x.dim() == 2:
            # tokens input, use embedding
            x = x.float()
        x = self.proj(x)
        x = self.pos_enc(x)
        return x
...
class GraphTransformerEncoder(nn.Module):
    def __init__(self, node_dim, edge_dim, hidden_dim=128, num_layers=3, heads=8):
        ...
        from torch_geometric.nn import GraphTransformer as GT
        self.gt = GT(
            node_dim=hidden_dim,
            edge_dim=hidden_dim,
...
def expand_mask_to_2d(Mask_B_init, NP):
    Mask_B = Mask_B_init.unsqueeze(2).expand(-1, -1, NP)
...
class PredictionMLP(nn.Module):
    def __init__(self, input_dim=256, hidden_dims=[1024, 512, 128], dropout=0.1):
        super().__init__()
        layers = []
        prev_dim = input_dim
...
class MAIDTA(nn.Module):
    def __init__(self, config):
        ...
        self.prot_seq_encoder = ESM1vEncoder(
            embed_dim=F,
            esm_dim=config.get('esm_dim', 1280)
        )

        self.drug_graph_encoder = GraphTransformerEncoder(
        ...
        self.prot_graph_encoder = GraphTransformerEncoder(
    ...
    def forward(self, drug_data, prot_data):
        X_M = self.drug_seq_encoder(drug_data['smiles_tokens'])

        X_E = self.prot_seq_encoder(prot_data['seq_tokens'])

        X_D_nodes = self.drug_graph_encoder(
            drug_data['graph_x'],
            drug_data['graph_edge_index'],
            drug_data['graph_edge_attr'],
            drug_data['batch']
        )
        X_D, _ = to_dense_batch(X_D_nodes, drug_data['batch'])

        X_P_nodes = self.prot_graph_encoder(
            prot_data['graph_x'],
            prot_data['graph_edge_index'],
            prot_data['graph_edge_attr'],
            prot_data['batch']
        )
        X_P, _ = to_dense_batch(X_P_nodes, prot_data['batch'])
