...
def create_parser():
    ...
    parser.add_argument(
        ...
        default="/tmp/pycharm_project_829/esm1v_t33_650M_UR90S_1.pt"
    )
    parser.add_argument(
        "-f",
        "--fasta-file",
...
def main(args):
    ...
    model, alphabet = pretrained.load_model_and_alphabet(str(args.model_location))
...
if __name__ == "__main__":
    class Args:
        model_location = pathlib.Path("/tmp/pycharm_project_829/esm1v_t33_650M_UR90S_1.pt")  # 可填完整路径如 /root/MDNN/models/esm1v_t33_650M_UR90S_1.pt
        fasta_file = pathlib.Path("dataset/kiba/original/kibaFASTA.fasta")
        output_dir = pathlib.Path("dataset/kiba_protein_feature/")
        run_all = False
