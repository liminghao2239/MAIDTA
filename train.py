...
from test import save_attention_matrices
...
class iEdgeTraining(object):
    ...
    def train(self, train_data, valid_data):
        model_file_name, model_state_name, result_train_name, result_eval_name, figure_file_name = self.SAVE_LOCATION(
            fname=self.FILENAME)
        ...
        train_loader, valid_loader, _ = self.LOAD_DATA(train=train_data, valid=valid_data, test=None,
        ...
        if self.RESUME == True:
            best_mse, best_epoch, start_epoch, optimizer, self.model, self.LR = self.GET_RESUME_PARAMS(
                models=self.model, optimizers=optimizer, model_states=model_state_name)
        ...
        for epoch in range(start_epoch, self.NUM_EPOCHS):
            ...
            self.LR = LR_scheduler_with_warmup(optimizer, self.LR, epoch, warmup_epoch=50, scale=0.8,
                                               set_LR=self.MAX_LR, interval_epoch=100)

            with open(result_train_name, 'a') as f:
                f.write("On epoch: " + str(epoch + 1) + ", LR : " + str(self.LR) + " ---> " + "Loss: " + str(
                    train_loss) + " Trainning ci = " + str(train_ci) + '\n')

            with open(result_eval_name, 'a') as f:
                f.write("On epoch" + str(epoch + 1) + ", Validation mse --> " + str(
                    metric[0]) + ", Validation ci --> " + str(metric[1]) + '\n')

            if metric[0] < best_mse:
                best_mse = metric[0]
                best_epoch = epoch + 1
                print("On epoch", best_epoch, ", Validation error decrease to --> ", best_mse, ", CI score --> ",
                      str(metric[1]))
                torch.save(self.model.state_dict(), model_file_name + '.model')  # 保存最优模型

        history = {"train_loss": train_loss_list, "train_ci": train_ci_list, "valid_loss": valid_loss_list,
                   "valid_ci": valid_ci_list}
        self.PLOT_LOSS(history=history, fignames=figure_file_name, start_epoch=3)
        torch.save(self.model.state_dict(), model_file_name + '_finale.model')  # 保存最终模型（最后一个epoch）
        ...
        print("\n. . . Training Complete . . .\n")
        print("Training complete, saving attention matrices...")
        save_attention_matrices(self.model.saved_attention_matrices, output_dir="attention_matrices")
    ...
    def eval(self, test_data, model_list):
        ...
        # Test set prediction
            ...
            df = pd.DataFrame({
            "（True Affinity）": ground_truth,
            "（Predicted Affinity）": prediction
            })

            excel_path = os.path.join(excel_dir, f"{m}_predictions.xlsx")
...
if __name__=="__main__":
    ...
    class Args:
        ...
        config = pathlib.Path("/root/MDNN/default_config.json")
